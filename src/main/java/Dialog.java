...
/**
 * This file is responsible for generating the pop-up windows and warnings for the GUI
 */
public class Dialog {
    /**
     ...
     * @return Mode
     */
    public static BattleshipProtocol.Mode selectMode(Component c) {
        Object[] options = {
                "Client",
                "Server"
        };
        int result = JOptionPane.showOptionDialog(c, "Please choose your mode", "Mode Selection", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, BattleshipProtocol.Mode.SERVER);
        if (result == JOptionPane.OK_OPTION) {
            return BattleshipProtocol.Mode.CLIENT;
        } else if (result == JOptionPane.NO_OPTION) {
            return BattleshipProtocol.Mode.SERVER;
        } else {
            return null;
        }
    }
